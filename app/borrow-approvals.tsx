...
export default function BorrowApprovalsPage() {
  ...
  const fetchRequests = async () => {
    try {
      const resp = await fetch(`${BASE_URL}/api/borrow-requests`);
      if (resp.ok) {
    ...
    const connectWebSocket = () => {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (message.type === 'NEW_BORROW_REQUEST') {
  ...
  const handleUpdateStatus = async (requestId: int, newStatus: string) => {
    ...
    if (confirm(`Bạn có chắc chắn muốn ${statusText} yêu cầu này?`)) {
      try {
        const resp = await fetch(`${BASE_URL}/api/borrow-requests/${requestId}/status`, {
          method: 'PUT',
